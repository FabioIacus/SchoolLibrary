...
if (isset($_GET["var"])) {
  ...
  <section>
  <article>
    <h2>Video caricati da Youtube</h2>
    <?php
    ...
    <table style="float:right">
      <tr>
        <td><a href="inseriscivideoyt.php" style="color:#091534"><button class="button1">Inserisci video</button></a>
        </td><td><a href="eliminavideoyt.php" style="color:#091534"><button class="button1">Elimina video</button></a>
        </td>
  </tr>
    </table>
    <br>
    ...
    $query = "select video_yt.src, video_yt.id, video_yt.descrizione, condividere.anno
              from video_yt, utenti, condividere
              where utenti.IDUtente = condividere.insegnante and condividere.video = video_yt.id and utenti.IDUtente = " . $_SESSION["idutente"] . "
              group by condividere.anno";     //query per stampare le informazione sul singolo anno
    $risultato = mysqli_query($connessione, $query);  //esegue query
    if (mysqli_num_rows($risultato) != 0) {
      while ($row = mysqli_fetch_array($risultato)) {
        echo "<button class='collapsible'>$row[anno]</button>";
        echo "<div class='content'>";
        $query2 = "select video_yt.id, video_yt.src, video_yt.descrizione, condividere.anno
                  from video_yt, utenti, condividere
                  where utenti.IDUtente = condividere.insegnante and condividere.video = video_yt.id and condividere.anno = ". $row["anno"] . " and utenti.IDUtente = " . $_SESSION["idutente"] . "
                  group by video_yt.id";     //query per stampare le informazione sul singolo video
        $risultato2 = mysqli_query($connessione, $query2);  //esegue query
        while ($row2 = mysqli_fetch_array($risultato2)) {
          $src = str_replace('watch?v=', 'embed/', $row2['src']);
          echo "<table>";
          echo "<tr>";
          echo "<td><iframe width='200' height='100' src='$src'></iframe></td>";
    ...
    if (mysqli_num_rows($risultato3) != 0){
      ...
      $query4 = "select video_yt.src, video_yt.id, video_yt.descrizione, condividere.anno
                from video_yt, utenti, condividere
                where utenti.IDUtente = condividere.insegnante and condividere.video = video_yt.id and utenti.IDUtente = $tmp
                group by condividere.anno";     //query per stampare le informazione sul singolo anno
      ...
      if (mysqli_num_rows($risultato4) != 0) {
        while ($row4 = mysqli_fetch_array($risultato4)) {
          ...
          echo "<div class='content'>";
          $query5 = "select video_yt.id, video_yt.src, video_yt.descrizione, condividere.anno
                    from video_yt, utenti, condividere
                    where utenti.IDUtente = condividere.insegnante and condividere.video = video_yt.id and condividere.anno = ". $row4["anno"] . " and utenti.IDUtente = $tmp
                    group by video_yt.id";     //query per stampare le informazione sul singolo video
          $risultato5 = mysqli_query($conn, $query5);  //esegue query
          while ($row5 = mysqli_fetch_array($risultato5)) {
            $src = str_replace('watch?v=', 'embed/', $row5['src']);
            echo "<table>";
            echo "<tr>";
            echo "<td><iframe width='200' height='100' src='$src'></iframe></td>";
  ...
  var f = true;
  window.location.href = "https://bibliotecalattanzio.000webhostapp.com/videoyt.php?var=" + x + "&ind=" + y + "&txt=" + z[y].text + "&f=" + f;
  }
  function indice0() {
