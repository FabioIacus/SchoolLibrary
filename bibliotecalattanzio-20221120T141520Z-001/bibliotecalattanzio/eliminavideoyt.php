...
<html>
    <head>
        <title>Eliminazione Video</title>
    ...
    <a style="cursor: pointer" class="black" onclick="javascript:history.go(-1)" data-toggle="tooltip" title="Torna indietro"><i class="material-icons">&#xe5c4;</i></a>
        ...
        <h1>Elimina video</h1>
			  <p>
        <form action="esegui.php?azione=ELIMINA_VIDEOYT" method="POST">
        <p>
				Seleziona video:
        <?php
        ...
        $query="SELECT * from condividere, video_yt where video_yt.id = condividere.video and condividere.insegnante = " . $_SESSION["idutente"] . " order by condividere.video";
        $risultato=mysqli_query($conn,$query);
        if (mysqli_num_rows($risultato) != 0){
          echo "<select name=\"id\" required>";
          while ($row=mysqli_fetch_array($risultato)) {
            echo "<option value=\"$row[video]\">$row[descrizione]&nbsp&nbsp&nbsp- Caricato nel $row[anno] -</option>";
          }
          echo "</select>";
        }
        if (mysqli_num_rows($risultato) == 0) {
