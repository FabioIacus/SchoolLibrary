...
if (isset($_GET["var"])) {
  ...
  <article>
    ...
    <table style="float:right">
      <tr>
    <td><a href="inseriscidocumento.php" style="color:#091534"><button class="button1">Inserisci documento</button></a>
    </td><td><a href="eliminadocumento.php" style="color:#091534"><button class="button1">Elimina documento</button></a>
    </td>
  </tr>
    </table>
    <br>
    ...
    $query = "select documenti.src, documenti.id, documenti.descrizione, condividere4.anno
              from documenti, utenti, condividere4
              where utenti.IDUtente = condividere4.insegnante and condividere4.documento = documenti.id and utenti.IDUtente = " . $_SESSION["idutente"] . "
              group by condividere4.anno";     //query per stampare le informazione sul singolo anno
    $risultato = mysqli_query($connessione, $query);  //esegue query
    if (mysqli_num_rows($risultato) != 0) {
      while ($row = mysqli_fetch_array($risultato)) {
        echo "<button class='collapsible'>$row[anno]</button>";
        echo "<div class='content'>";
        $query2 = "select documenti.id, documenti.src, documenti.descrizione, condividere4.anno
                  from documenti, utenti, condividere4
                  where utenti.IDUtente = condividere4.insegnante and condividere4.documento = documenti.id and condividere4.anno = ". $row["anno"] . " and utenti.IDUtente = " . $_SESSION["idutente"] . "
                  group by documenti.id";     //query per stampare le informazione sul singolo video
        ...
        while ($row2 = mysqli_fetch_array($risultato2)) {
          echo "<tr>";
          echo "<td class='left'><a href='documenti/$row2[src]' target='_blank'>Scarica file</a></td>";
    ...
    if (mysqli_num_rows($risultato3) != 0){
      ...
      $query4 = "select documenti.src, documenti.id, documenti.descrizione, condividere4.anno
                from documenti, utenti, condividere4
                where utenti.IDUtente = condividere4.insegnante and condividere4.documento = documenti.id and utenti.IDUtente = $tmp
                group by condividere4.anno";     //query per stampare le informazione sul singolo anno
      ...
      if (mysqli_num_rows($risultato4) != 0) {
        while ($row4 = mysqli_fetch_array($risultato4)) {
          ...
          echo "<div class='content'>";
          $query5 = "select documenti.id, documenti.src, documenti.descrizione, condividere4.anno
                    from documenti, utenti, condividere4
                    where utenti.IDUtente = condividere4.insegnante and condividere4.documento = documenti.id and condividere4.anno = ". $row4["anno"] . " and utenti.IDUtente = $tmp
                    group by documenti.id";     //query per stampare le informazione sul singolo video
          $risultato5 = mysqli_query($conn, $query5);  //esegue query
            ...
            while ($row5 = mysqli_fetch_array($risultato5)) {
              echo "<tr>";
              echo "<td class='left'><a href='documenti/$row5[src]' target='_blank'>Scarica file</a></td>";
  ...
  var f = true;
  window.location.href = "https://bibliotecalattanzio.000webhostapp.com/documenti.php?var=" + x + "&ind=" + y + "&txt=" + z[y].text + "&f=" + f;
  }
  function indice0() {
