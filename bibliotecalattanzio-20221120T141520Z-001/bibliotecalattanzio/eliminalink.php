...
<html>
    ...
    <a style="cursor: pointer" class="black" onclick="javascript:history.go(-1)" data-toggle="tooltip" title="Torna indietro"><i class="material-icons">&#xe5c4;</i></a>
        ...
        <h1>Elimina link</h1>
			  <p>
        <form action="esegui.php?azione=ELIMINA_LINK" method="POST">
        <p>
				Seleziona link:
        <?php
        ...
        $query="SELECT * from condividere2, sitografie where sitografie.id = condividere2.sitografia and condividere2.insegnante = " . $_SESSION["idutente"] . " order by condividere2.sitografia";
        $risultato=mysqli_query($conn,$query);
        if (mysqli_num_rows($risultato) != 0){
          echo "<select name=\"id\" required>";
          while ($row=mysqli_fetch_array($risultato)) {
            echo "<option value=\"$row[sitografia]\">$row[descrizione]&nbsp&nbsp&nbsp- Caricato nel $row[anno] -</option>";
          }
          echo "</select>";
        }
        if (mysqli_num_rows($risultato) == 0) {
