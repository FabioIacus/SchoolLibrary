...
<html>
    ...
    <a style="cursor: pointer" class="black" onclick="javascript:history.go(-1)" data-toggle="tooltip" title="Torna indietro"><i class="material-icons">&#xe5c4;</i></a>
        ...
        <h1>Inserimento prestito</h1>
			...
			<p>
                        <form action="esegui.php?azione=AGGIUNGI_PRESTITO" method="POST">
								<p>
                                Seleziona l'utente:
								<?php
                                ...
                                $query="SELECT * from utenti where utenti.IDIstituto = '$_SESSION[istituto]' order by nome";
                                $risultato=mysqli_query($conn,$query);
                                if (mysqli_num_rows($risultato) != 0)
                                {
                                    echo "<select name='utente' selected>";
