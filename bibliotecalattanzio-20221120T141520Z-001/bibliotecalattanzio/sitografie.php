...
if (isset($_GET["var"])) {
  ...
  <article>
    ...
    <table style="float:right">
      <tr>
    <td><a href="inseriscilink.php" style="color:#091534"><button class="button1">Inserisci link</button></a>
    </td><td><a href="eliminalink.php" style="color:#091534"><button class="button1">Elimina link</button></a>
    </td>
  </tr>
    </table>
    <br>
    ...
    $query = "select sitografie.src, sitografie.id, sitografie.descrizione, condividere2.anno
              from sitografie, utenti, condividere2
              where utenti.IDUtente = condividere2.insegnante and condividere2.sitografia = sitografie.id and utenti.IDUtente = " . $_SESSION["idutente"] . "
              group by condividere2.anno";     //query per stampare le informazione sul singolo anno
    $risultato = mysqli_query($connessione, $query);  //esegue query
    if (mysqli_num_rows($risultato) != 0) {
      while ($row = mysqli_fetch_array($risultato)) {
        echo "<button class='collapsible'>$row[anno]</button>";
        echo "<div class='content'>";
        $query2 = "select sitografie.id, sitografie.src, sitografie.descrizione, condividere2.anno
                  from sitografie, utenti, condividere2
                  where utenti.IDUtente = condividere2.insegnante and condividere2.sitografia = sitografie.id and condividere2.anno = ". $row["anno"] . " and utenti.IDUtente = " . $_SESSION["idutente"] . "
                  group by sitografie.id";     //query per stampare le informazione sul singolo video
        $risultato2 = mysqli_query($connessione, $query2);  //esegue query
        echo "<table>";
        echo "<tr>";
        echo "<th class='left'>Link</th><th>Descrizione</th>";
        echo "</tr>";
        while ($row2 = mysqli_fetch_array($risultato2)) {
          echo "<tr>";
          echo "<td class='left'><a href='$row2[src]' target='_blank'>Vai al sito web :)</a></td>";
          echo "<td class='right'>$row2[descrizione]</td>";
          echo "</tr>";
        }
        echo "</table>";
      echo "</div>";
    ...
    if (mysqli_num_rows($risultato3) != 0){
      ...
      $query4 = "select sitografie.src, sitografie.id, sitografie.descrizione, condividere2.anno
                from sitografie, utenti, condividere2
                where utenti.IDUtente = condividere2.insegnante and condividere2.sitografia = sitografie.id and utenti.IDUtente = $tmp
                group by condividere2.anno";     //query per stampare le informazione sul singolo anno
      ...
      if (mysqli_num_rows($risultato4) != 0) {
        while ($row4 = mysqli_fetch_array($risultato4)) {
          ...
          echo "<div class='content'>";
          $query5 = "select sitografie.id, sitografie.src, sitografie.descrizione, condividere2.anno
                    from sitografie, utenti, condividere2
                    where utenti.IDUtente = condividere2.insegnante and condividere2.sitografia = sitografie.id and condividere2.anno = ". $row4["anno"] . " and utenti.IDUtente = $tmp
                    group by sitografie.id";     //query per stampare le informazione sul singolo video
          $risultato5 = mysqli_query($conn, $query5);  //esegue query
            ...
            while ($row5 = mysqli_fetch_array($risultato5)) {
              echo "<tr>";
              echo "<td class='left'><a href='$row5[src]' target='_blank'>Vai al sito web :)</a></td>";
              echo "<td class='right'>$row5[descrizione]</td>";
              echo "</tr>";
            }
            echo "</table>";
              echo "</div>";
  ...
  var f = true;
  window.location.href = "https://bibliotecalattanzio.000webhostapp.com/sitografie.php?var=" + x + "&ind=" + y + "&txt=" + z[y].text + "&f=" + f;
  }
  function indice0() {
