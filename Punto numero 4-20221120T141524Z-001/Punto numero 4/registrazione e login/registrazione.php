	...
	<article>
		...
		if (isset($_GET["verifica"])) {
			if ($_GET["verifica"] == "si") {
				...
				$controllausername = "SELECT utenti.Username from utenti where utenti.Username = '$_POST[username]'";
				$prendi = mysqli_query($conn, $controllausername);
				if (mysqli_fetch_array($prendi) == 0) {
	        ...
	        $codice = $_POST['codice'];
					...
					runSql($sql2,$conn);
					$query3 = "select istituto.studenti_portale from istituto where istituto.id = '$row[id]' ";
					$risultato3 = mysqli_query($conn,$query3);
					$row3 = mysqli_fetch_array($risultato3);
					$sql3="UPDATE istituto SET studenti_portale = ($row3[studenti_portale]) + 1 WHERE id ='$row[id]'";
					runSql($sql3,$conn);
					mysqli_close($conn);
					echo "<b><p style='color: green; text-align:center; margin-top: 0em;'>Registrazione avvenuta con successo!</p></b>";
					echo "<b><p style='color: green; text-align:center; margin-top: 0em;'>Adesso puoi effettuare il login</p></b>";
				...
				echo "<b><p style='color: red; text-align:center; margin-top: 0em;'>Username inserito già esistente!</p></b>";
			}
			}
		}
		?>
